...
import { questionSig, correctionApplies, optionMatchesCorrect, findLoose, extractClozeSequence, orderedOptionsInText, topOptionByFrequency, extractOrderFromFeedbackText, extractOrderFromCorrectListText, bestOverlapIndex, resolveWithElimination, assignMatchingPairs, extractPairs, extractOrder, permute, buildClozeCandidates, arrangements, planOrderingTaps, synonymOverlapIndex, parsePairsFromCorrectCombo, wrongOptionsInExplanation, clozeSeqFromFeedbackText } from "./drill-review-core.mjs";
...
t("buildClozeCandidates: 正しい導出が先頭・かつ全順列(6件)を含む（ブランチ L4 Q9 実データ）", () => {
  ...
  // 先頭6件＝充填集合[どんな実験,main,安全な]の全順列（導出集合が正しければ6手以内に収束）。
  // 7件目以降は解説出現語の総当たり順列（集合ミス時の保険＝チーム開発 L6 Q7 対策）が続く。
  const head = new Set(cands.slice(0, 6).map((c) => c.join("¦")));
  assert.equal(head.size, 6);
  for (const p of permute(["どんな実験", "main", "安全な"])) assert.ok(head.has(p.join("¦")));
...
// 複数空欄で導出“集合”が誤りでも、解説に出現する選択肢の順列を候補に足して総当たりで収束する。
// 実ライブ チーム開発コース L6 Q7：解説「pull は fetch と merge をまとめて…push で送り返す」の出現順が
// distractor『merge』で汚れ [pull, merge] と誤導出→2件（順列）とも不正解で8回停止していた回帰ケース。
const clozeExplTeamL6Q7 =
  "## 問題\nfeatureブランチでリモートの取得とマージをまとめて行う `git ＿＿＿ origin main` を実行し、衝突箇所を解決・コミット後に `git ＿＿＿ ` でPRを更新する。\n\n## 回答\n選択問題です。\n\n選択肢:\n- push\n- merge\n- add\n- commit\n- pull\n\n## 解説\n### なぜこれが正解？\nリモートから取得してマージする2つの処理が必要です。pull（プル）はこの2つをまとめて実行するので、fetch（フェッチ）とmerge（マージ）を分ける手間が省けます。衝突を解決してコミットした後、push（プッシュ）でリモートに送り返すとPRが更新されるわけです。\n\n### 間違い選択肢のどこが違う？\nmerge：リモートから取ってくる処理が抜けます。";
const clozeOptsTeamL6Q7 = ["push", "pull", "commit", "add", "merge"];
t("buildClozeCandidates: 複数空欄で導出集合が誤りでも解説出現語の順列で正解[pull,push]へ収束（チーム開発 L6 Q7）", () => {
  const cands = buildClozeCandidates(clozeExplTeamL6Q7, clozeOptsTeamL6Q7, 2);
  const keys = new Set(cands.map((c) => c.join("¦")));
  assert.ok(keys.has("pull¦push"), "正解順[pull,push]が候補に含まれる");
  // clozeTried で未試行を辿れば MAX_ATTEMPTS(8) 以内に正解へ到達する。
  const tried = new Set();
  let answer = null, attempts = 0;
  for (let i = 0; i < cands.length; i++) {
    const pick = cands.find((c) => !tried.has(c.join("¦")));
    if (!pick) break;
    attempts += 1;
    if (pick.join("¦") === "pull¦push") { answer = pick; break; }
    tried.add(pick.join("¦"));
  }
  assert.deepEqual(answer, ["pull", "push"]);
  assert.ok(attempts <= 8, `MAX_ATTEMPTS(8)以内に収束（実際${attempts}回目）`);
});

// arrangements: pool から重複なく k 個を選ぶ順列を出現順優先で列挙する。
t("arrangements: 3語から2個の順列を6通り・先頭要素優先の順で返す", () => {
  const a = arrangements(["x", "y", "z"], 2);
  assert.equal(a.length, 6);
  assert.deepEqual(a[0], ["x", "y"]);
  assert.deepEqual(a[1], ["x", "z"]);
  assert.ok(a.every((p) => p[0] !== p[1]), "同じ語を2回使わない");
});
...
// 「正しい順番」番号付きリスト（2026-07-04 実ライブ UIデザインの世界 L1 復習 Q4 の review-wrong-Q4.html 実テキスト）。
// このブロックは quiz-feedback の外側（回答エリア）に出て、ワンポイント本文には順序ヒントが一切無い。
t("「正しい順番」番号リスト（行ベース・innerText 実形式）から正解順を抽出", () => {
  const pageText = [
    "復習中", "Q4", "/ 8問",
    "スマホの地図アプリで「気になるお店を調べる」とき、次の操作を正しい順番に並べてください。",
    "正しい順番に並べてください",
    "検索結果からお店を選ぶ", "お店の詳細情報を確認する", "ホーム画面で地図アプリを開く", "検索バーにお店の名前を入力する",
    "正しい順番",
    "1", "ホーム画面で地図アプリを開く",
    "2", "検索バーにお店の名前を入力する",
    "3", "検索結果からお店を選ぶ",
    ...
    "不正解...",
    "マスターのワンポイント",
    "何気なくこなしているこの操作、実はすべての場面で画面が「次はこれをしてね」と案内しています。",
  ].join("\n");
  assert.deepEqual(extractOrderFromCorrectListText(pageText), [
    "ホーム画面で地図アプリを開く", "検索バーにお店の名前を入力する", "検索結果からお店を選ぶ", "お店の詳細情報を確認する",
  ]);
});
t("「正しい順番」番号リスト（空白1行つづき形式）でも抽出できる", () => {
  const flat =
    "正しい順番に並べてください 検索結果からお店を選ぶ ホーム画面で地図アプリを開く " +
    "正しい順番 1 ホーム画面で地図アプリを開く 2 検索バーにお店の名前を入力する 3 検索結果からお店を選ぶ 4 お店の詳細情報を確認する 不正解... マスターのワンポイント";
  assert.deepEqual(extractOrderFromCorrectListText(flat), [
    "ホーム画面で地図アプリを開く", "検索バーにお店の名前を入力する", "検索結果からお店を選ぶ", "お店の詳細情報を確認する",
  ]);
});
t("指示文「正しい順番に並べてください」だけではリスト扱いしない（誤学習防止）", () => {
  assert.deepEqual(extractOrderFromCorrectListText("正しい順番に並べてください\nA\nB\nC\n不正解..."), []);
  assert.deepEqual(extractOrderFromCorrectListText("不正解... マスターのワンポイント 順序の説明なし"), []);
  assert.deepEqual(extractOrderFromCorrectListText(""), []);
});
t("番号が 1 から昇順連続でない場合は信頼しない", () => {
  assert.deepEqual(extractOrderFromCorrectListText("正しい順番\n2\nB\n3\nC\n不正解..."), []);
});
...
console.log("\nextractOrder: 1行矢印チェーン形式（ビルドの裏側 L8 Q9 実データ＝シリーズ停止の原因）");
t("1行「1. **A** → 2. **B** → 3. **C**」を3手順に分割し番号・**を剥がす", () => {
  const expl = "## 解説\n### 正しい順序\n\n1. **pnpm install** → 2. **pnpm build** → 3. **CDN(コンテンツデリバリーネットワーク)配信**\n\n### なぜこの順序なのか\n本文…";
  assert.deepEqual(extractOrder(expl), [
    "pnpm install",
    "pnpm build",
    "CDN(コンテンツデリバリーネットワーク)配信",
  ]);
});
t("従来の複数行リストは分割しない（手順内の矢印も保持）", () => {
  const expl = "### 正しい順序\n1. コードを push する\n2. ローカル → リモートへ反映を確認\n3. デプロイする\n";
  assert.deepEqual(extractOrder(expl), [
    "コードを push する",
    "ローカル → リモートへ反映を確認",
...
t("複数行リストの **強調** は剥がす", () => {
  const expl = "### 正しい順序\n1. **pnpm install**\n2. **pnpm build**\n";
  assert.deepEqual(extractOrder(expl), ["pnpm install", "pnpm build"]);
});

console.log("\nparsePairsFromCorrectCombo: 誤答フィードバック「正しい組み合わせ」からペア学習（DBがないと壊れる世界 L1 復習Q4 実データ＝旧スクショ取込で照合全滅時の停止原因）");
const comboLefts = ["SNS", "ECサイト", "レストランの予約サイト"];
t("見出し＋交互行＋「3/3 ペア完成」の実DOM並びから3ペアを組む", () => {
  const lines = ["正しい組み合わせ", "SNS", "投稿文・画像・いいね数", "ECサイト", "商品名・価格・在庫数", "レストランの予約サイト", "日時・人数・テーブル番号", "3/3 ペア完成"];
  assert.deepEqual(parsePairsFromCorrectCombo(lines, comboLefts), [
    { left: "SNS", right: "投稿文・画像・いいね数" },
    { left: "ECサイト", right: "商品名・価格・在庫数" },
    { left: "レストランの予約サイト", right: "日時・人数・テーブル番号" },
  ]);
});
t("見出しより前の行（問題盤面の混入）は無視する", () => {
  const lines = ["それぞれのサービスが…", "SNS", "商品名・価格・在庫数", "正しい組み合わせ", "SNS", "投稿文・画像・いいね数", "ECサイト", "商品名・価格・在庫数", "レストランの予約サイト", "日時・人数・テーブル番号"];
  const pairs = parsePairsFromCorrectCombo(lines, comboLefts);
  assert.equal(pairs.length, 3);
  assert.deepEqual(pairs[0], { left: "SNS", right: "投稿文・画像・いいね数" });
});
t("「ペア完成」以降の別UI行は取り込まない", () => {
  const lines = ["正しい組み合わせ", "SNS", "投稿文・画像・いいね数", "3/3 ペア完成", "ECサイト", "商品名・価格・在庫数"];
  assert.deepEqual(parsePairsFromCorrectCombo(lines, comboLefts), [{ left: "SNS", right: "投稿文・画像・いいね数" }]);
});
t("ペア間のアイコングリフ行（私用領域文字＝見た目は空）は右ラベルに化けない", () => {
  // 実DOM検証で発見: ionicons の矢印グリフ(U+F30F等)だけの行が左右の間に挟まる
  const lines = ["正しい組み合わせ", "SNS", "", "投稿文・画像・いいね数", "ECサイト", "", "商品名・価格・在庫数", "レストランの予約サイト", "", "日時・人数・テーブル番号"];
  assert.deepEqual(parsePairsFromCorrectCombo(lines, comboLefts), [
    { left: "SNS", right: "投稿文・画像・いいね数" },
    { left: "ECサイト", right: "商品名・価格・在庫数" },
    { left: "レストランの予約サイト", right: "日時・人数・テーブル番号" },
...
t("左ラベルの表記ゆれで判定できない時は交互並びフォールバックで全ペアを組む", () => {
  const lines = ["正しい組み合わせ", "SNS（エスエヌエス）", "投稿文・画像・いいね数", "EC サイト!", "商品名・価格・在庫数", "予約サイト", "日時・人数・テーブル番号"];
  assert.deepEqual(parsePairsFromCorrectCombo(lines, comboLefts), [
    { left: "SNS（エスエヌエス）", right: "投稿文・画像・いいね数" },
    { left: "EC サイト!", right: "商品名・価格・在庫数" },
    { left: "予約サイト", right: "日時・人数・テーブル番号" },
  ]);
});
t("学習ペアは assignMatchingPairs で実セルへ割当できる（自己訂正の一気通貫）", () => {
  const lines = ["正しい組み合わせ", "SNS", "投稿文・画像・いいね数", "ECサイト", "商品名・価格・在庫数", "レストランの予約サイト", "日時・人数・テーブル番号"];
  const pairs = parsePairsFromCorrectCombo(lines, comboLefts);
  const rightItems = ["投稿文・画像・いいね数", "日時・人数・テーブル番号", "商品名・価格・在庫数"];
  const plan = assignMatchingPairs(comboLefts, rightItems, pairs);
  assert.ok(plan, "plan が得られる");
  assert.deepEqual(plan, [
    [0, "投稿文・画像・いいね数"],
    [1, "商品名・価格・在庫数"],
    [2, "日時・人数・テーブル番号"],
...
console.log("\n穴埋め: 正解語が本文に表記どおり現れず誤答だけ太字見出しで載る解説（ネット接続とクラウドの入口 L3 Q6 実データ＝候補2件で尽きて8回停止）");
const clozeExplNetL3Q6 = `## 問題
Wi-Fi が遅いとき、まず「家の他の端末はどうか」を確認します。全員遅ければ、原因はどこにありそうですか。
...
## 解説
### なぜこれが正解？
家全体の複数の端末が全員遅い状況というのは、各端末の個別の問題ではなく、すべての通信が通る共通経路の問題を指しています。その共通経路とはルーターか、ルーターの先にある回線のどちらかです。だから答えはこの2つに絞られます。

### 間違い選択肢のどこが違う？
**サービス側**：YouTubeやNetflixなどのサービス自体が遅い場合、あなたの家の別の端末がそのサービスを使わなければ、その端末は遅くなりません。つまり「全員遅い」という状況が起こりません。

**端末側**：あなたのスマートフォンだけが遅いのなら端末側が原因ですが、全員遅いなら個別の端末は犯人ではありません。

### 覚えるポイント
1. **共通経路を考える**：複数の人が同じ問題を経験したら、みんなの通信が通る共通の部分を疑うことが故障診断の基本です。

## 用語
- 回線(かいせん): インターネット会社から家まで来ている、データの通り道`;
const clozeOptsNetL3Q6 = ["回線側", "ルーター側", "サービス側", "端末側"];
t("wrongOptionsInExplanation: 間違い選択肢節の太字見出しだけを誤答と判定する", () => {
  const wrong = wrongOptionsInExplanation(clozeExplNetL3Q6, clozeOptsNetL3Q6);
  assert.deepEqual([...wrong].sort(), ["サービス側", "端末側"]);
});
t("extractClozeSequence: 誤答集合[サービス側,端末側]を導出しない（読めない時は未知=[]に倒す）", () => {
  assert.deepEqual(extractClozeSequence(clozeExplNetL3Q6, clozeOptsNetL3Q6, 2), []);
});
t("buildClozeCandidates: 誤答除外後の残り2語の順列が先頭候補に来る（誤答語入りは最終保険 tier まで出ない）", () => {
  const cands = buildClozeCandidates(clozeExplNetL3Q6, clozeOptsNetL3Q6, 2);
  assert.ok(cands.some((c) => c.join("¦") === "ルーター側¦回線側"), "正解順が候補にある");
  // 早期候補（先頭2件＝除外後2語の順列）に誤答語は混ざらない。全選択肢順列の最終保険 tier は
  // 「間違い選択肢節が正解語を誤掲載する」ケース（文字と余白L7 筆書体）の救済用で、末尾にのみ足される。
  const early = cands.slice(0, 2);
  assert.ok(early.every((c) => !c.includes("サービス側") && !c.includes("端末側")), "誤答語は早期候補に混ざらない");
  const correctIdx = cands.findIndex((c) => c.join("¦") === "ルーター側¦回線側");
  assert.ok(correctIdx <= 1, `正解順は先頭2件以内（実際: ${correctIdx + 1}件目）`);
});
t("clozeSeqFromFeedbackText: ワンポイントが正解2語の後に対比で誤答語に触れても先頭2語を採る（実フィードバック文）", () => {
  const fb = "不正解...\nマスターのワンポイント\n全員遅いということは、共通の経路である ルーター側 か、その先の 回線側 に問題があります。\n端末側なら「自分だけ遅い」になるはずです。";
  assert.deepEqual(clozeSeqFromFeedbackText(fb, clozeOptsNetL3Q6, 2), ["ルーター側", "回線側"]);
});
t("clozeSeqFromFeedbackText: 空欄数に満たない時は誤学習を避けて null", () => {
  assert.equal(clozeSeqFromFeedbackText("ワンポイント\nルーター側だけの言及", clozeOptsNetL3Q6, 2), null);
});
// 正解語が解説の地の文に一度も出ない穴埋め（2026-07-04 実ライブ 文字と余白のルール L7
// 「格調を出したい招待状には筆書体、画面やメニューはゴシック体」）。本文出現ベースの母集合が
// [明朝体,ゴシック体]に閉じて候補2件で尽き、フィードバック学習も同じ誤集合を読んで14回停止した。
t("buildClozeCandidates: 母集合が誤っていても全選択肢順列の最終候補に正解が含まれる", () => {
  const expl =
    "## 解説\n書体の使い分けが大切です。明朝体は長文の紙媒体に、ゴシック体は画面に向きます。\n" +
    "明朝体とゴシック体の違いを理解しましょう。";
  const options = ["筆書体", "ゴシック体", "丸ゴシック体", "明朝体"];
  const cands = buildClozeCandidates(expl, options, 2, null);
  const has = (a, b) => cands.some((c) => c[0] === a && c[1] === b);
  assert.ok(has("筆書体", "ゴシック体"), "正解[筆書体,ゴシック体]が候補に含まれること");
  assert.ok(cands.length >= 12, `全選択肢順列を含み12候補以上（実際: ${cands.length}）`);
});

t("buildClozeCandidates: 学習済み正解の順列も候補に足される（集合は正しく順序だけ違う場合に収束）", () => {
  const cands = buildClozeCandidates(clozeExplNetL3Q6, clozeOptsNetL3Q6, 2, ["回線側", "ルーター側"]);
  assert.deepEqual(cands[0], ["回線側", "ルーター側"], "学習済みが最優先");
  assert.ok(cands.some((c) => c.join("¦") === "ルーター側¦回線側"), "学習済みの逆順も候補にある");
});
