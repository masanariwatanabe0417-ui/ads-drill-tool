...
export async function POST(req: Request) {
  try {
    const {
      ...
      drillCatalog, // ツールに収録済みのドリル一覧（シリーズ→コース→レッスン名）。学習箇所の案内を実在の名前で行うため
    } = await req.json();
...
${drillCatalog ? `
【このツールに収録されているドリル一覧（シリーズ→コース: レッスン）】
${drillCatalog}

ドリル一覧の使い方：
- 「どこを勉強すればよいか」「関連するドリルはどれか」など学習箇所を勧める質問のときだけ参照する（それ以外の質問では無視する）
- 勧めるときは必ずこの一覧に実在するシリーズ名・コース名・レッスン名をそのまま正確に挙げる（例：「Web開発基礎シリーズ ＞ Webの世界 ＞ Lesson 1 Webの世界一周ツアー」の形式）
- コース名だけで終わらせず、質問内容に最も関係の深いレッスン名まで特定して挙げる（生徒がそのレッスンをすぐ開けるように）
- 一覧にない名前を挙げない（存在しないコース名・レッスン名を作らない）
` : ""}
【生徒からの質問】
${question}
...
以下のJSON形式で回答してください。JSONのみを返し、他のテキストは含めないでください：
{
  "answer": "質問への丁寧で分かりやすい回答（250字以内。おすすめのドリル箇所を挙げる場合は350字まで可）。英語・コード用語には必ずカタカナを括弧で補足してください",
  "proposedInsight": "このQ&Aで生徒が得た気づきを、生徒自身の言葉のような一人称の気づきメモ調で1〜3文にまとめる（例：「〜だと思っていたが、実は〜だと分かった」「〜と〜は…という点でつながっていた」）。まとめに既に書いてあることの言い換えではなく、質問を通じて新しく腑に落ちた点を書くこと。回答でおすすめのドリル箇所を挙げた場合は、そのシリーズ名・コース名・レッスン名をそのまま気づき文にも含めること（後から読み返してすぐそのドリルを開けるように）。気づきと呼べる内容がなければ空文字にする。英語用語はカタカナ補足。"
}`
      : `あなたは入社したての社員に教える親切な先輩社員です。
レッスン：${lessonTitle}
